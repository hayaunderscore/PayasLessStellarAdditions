...
SMODS.Blind {
	...
	collection_loc_vars = function(self)
		return { key = "bl_plsa_question_alt" }
	end,
	loc_vars = function(self)
		if G.GAME.plsa_merged_boss_keys and next(G.GAME.plsa_merged_boss_keys) then
			local localized = {}
			for i = 1, #G.GAME.plsa_merged_boss_keys do
				localized[#localized + 1] = localize { type = 'name_text', key = G.GAME.plsa_merged_boss_keys[i], set = 'Blind' }
			end
	...
	set_blind = function(self)
		...
		if not G.GAME.plsa_merged_boss_keys then G.GAME.plsa_merged_boss_keys = {} end
	...
	defeat = function(self)
		...
		G.GAME.plsa_merged_boss_keys = nil
	end,
	...
	calculate = function(self, blind, context)
		...
		for k, v in ipairs(G.GAME.plsa_merged_boss_keys) do
			...
			if G.P_BLINDS[v].calculate then
				...
				if calc and type(calc) == "table" then SMODS.merge_effects(effects, calc) end
			end
		end
...
function create_UIBox_blind_fused(blind)
	local fused_lines = {}
	for _, v in ipairs(G.GAME.plsa_merged_boss_keys) do
...
function Blind.hover(self)
	if not G.CONTROLLER.dragging.target or G.CONTROLLER.using_touch then
		if not self.hovering and self.states.visible and self.children.animatedSprite.states.visible then
			if G.GAME.plsa_merged_boss_keys and next(G.GAME.plsa_merged_boss_keys) and self.config.blind and self.config.blind.show_fusions then
				G.blind_fused = UIBox {
					...
					config = {
						...
						offset = {
							x = 0.15,
							y = 0.2 + 0.38 * #G.GAME.plsa_merged_boss_keys,
						},
						type = "cr",
					}
				}
